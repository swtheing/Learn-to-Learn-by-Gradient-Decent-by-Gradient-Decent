...
class FastMetaOptimizer(nn.Module):
    ...
    def meta_update(self, model_with_grads, loss):
        ...
        inputs = Variable(torch.cat((preprocess_gradients(flat_grads), flat_params.data.view(-1, 1), loss), 1))
        inputs = torch.cat((inputs, self.f, self.i), 1)
        self.f, self.i = self(inputs)

        # Meta update itself
        # f is a matrix (flat_params.shape[0], 1)
        flat_params = self.f[:,0] * flat_params - self.i[:,0] * Variable(flat_grads)
